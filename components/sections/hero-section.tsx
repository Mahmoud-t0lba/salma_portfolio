...
import { heroHighlights, siteConfig, socialLinks } from "@/content/site";
import { withBasePath } from "@/lib/base-path";

const domainHighlights = ["Transport", "Social", "Education", "Utilities"];
...
export function HeroSection() {
  const profileImageSrc = withBasePath("/images/mahmoud-profile.jpg");

  return (
    <section id="home" className="relative overflow-hidden pt-14 sm:pt-18">
      <div className="absolute inset-x-0 top-0 -z-10 h-[34rem] bg-[radial-gradient(circle_at_top_right,rgba(48,94,132,0.12),transparent_34%),radial-gradient(circle_at_top_left,rgba(255,255,255,0.8),transparent_28%)]" />
      <div className="mx-auto grid w-full max-w-6xl gap-10 px-5 pb-14 pt-10 sm:px-6 md:pb-18 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:pb-24 lg:pt-14">
        <Reveal className="max-w-3xl">
          <div className="surface-soft-lift inline-flex items-center rounded-full border border-[var(--line-strong)] bg-white/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-[var(--text-secondary)] shadow-[var(--shadow-soft)]">
            {siteConfig.status}
          </div>
          <h1 className="mt-7 font-display text-4xl tracking-[-0.06em] text-[var(--text-primary)] sm:text-5xl lg:text-[4.65rem] lg:leading-[0.96]">
            {siteConfig.headline}
          </h1>
        ...
        <Reveal className="lg:pt-10" delay={0.12} from="right">
          <Float delay={0.65} distance={8}>
            <div className="surface-interactive rounded-[2rem] border border-[var(--line)] bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(251,249,244,0.92))] p-6 shadow-[var(--shadow-strong)] sm:p-7">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-3 font-display text-2xl tracking-[-0.04em] text-[var(--text-primary)]">
                    Senior Flutter developer with architecture discipline.
                  </h2>
                </div>
              ...
              <div className="surface-soft-lift mt-6 rounded-[1.5rem] border border-[var(--line)] bg-[var(--surface-strong)] p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--text-muted)]">
                  What recruiters see fast
                </p>
                <ul className="mt-4 space-y-3 text-sm leading-6 text-[var(--text-secondary)]">
                  <li>Senior Flutter Developer with ownership across architecture, reviews, and team workflows.</li>
                  <li>Experienced across transport, social, education, and utility-focused mobile products.</li>
                  <li>Combines technical depth with product collaboration and practical execution.</li>
                </ul>
              </div>
